...
var __decorate, Il2Cpp2, Il2Cpp2, Il2Cpp2, Il2Cpp2, Il2Cpp2, Il2Cpp2, Il2Cpp2, Android, NativeStruct, UnityVersion, Il2Cpp2, Il2Cpp2, Il2Cpp2, Il2Cpp2, Il2Cpp2, Il2Cpp2, Il2Cpp2, Il2Cpp2, Il2Cpp2, Il2Cpp2, Il2Cpp2, Il2Cpp2, Il2Cpp2, Il2Cpp2, Il2Cpp2, Il2Cpp2, Il2Cpp2, Il2Cpp2, Il2Cpp2, Il2Cpp2, Il2Cpp2, Il2Cpp2, Il2Cpp2;
var init_dist = __esm({
  "node_modules/frida-il2cpp-bridge/dist/index.js"() {
    "use strict";
    init_node_globals();
...
var require_security = __commonJS({
  "security.ts"(exports) {
    ...
    var SecurityManager = class {
       ...
       * IMPORTANT:
       *
       * این Set یک Whitelist قطعی است.
       *
       * اگر نام Library اینجا باشد،
       * checkModule() و ModuleObserver
       * دیگر هیچ Suspicion یا Violation
       * برای آن ایجاد نمی‌کنند.
       *
       * مسیر اهمیتی ندارد.
       * ============================================================
       */
      allowedLibraries = /* @__PURE__ */ new Set([
        "libapminsighta.so",
        "libapminsightb.so",
        "libapplovin-native-crash-reporter.so",
        "libbuffer_pgl.so",
        "libfile_lock_pgl.so",
        "libsonic-xxxx.config.so",
        "libsonic-xxxx.so",
        "libil2cpp.so",
        "libmain.so",
        "libnms.so",
       ...
       * ============================================================
       *
       * اگر نام اینجا باشد و در Whitelist نباشد،
       * suspicious می‌شود.
       *
       * IMPORTANT:
       *
       * Whitelist اول بررسی می‌شود.
       * ============================================================
       */
      highRiskNames = /* @__PURE__ */ new Set([
        "libtool.so",
        "libgameguardian.so",
        "libgg.so",
        "libfrida-agent.so",
      ...
      /*
       * ============================================================
       * PRIVATE APP LOCATIONS
       * ============================================================
       *
       * چیزی که اینجا ظاهر شود ولی Whitelist نباشد،
       * اهمیت امنیتی دارد.
       * ============================================================
       */
      privateAppPrefixes = [
       ...
       * ============================================================
       *
       * Baseline فقط برای اطلاعات است.
       *
       * Unknown library فقط به خاطر اینکه
       * قبل از شروع Security وجود داشته،
       * trusted نمی‌شود.
       * ============================================================
       */
      baseline = /* @__PURE__ */ new Map();
      suspicious = /* @__PURE__ */ new Map();
      /*
       * ============================================================
       * HELPERS
       * ============================================================
      ...
      moduleKey(name, path) {
        return this.normalize(name) + "|" + this.normalize(path);
      }
      isSo(name, path) {
        const n = this.normalize(name);
        const p = this.normalize(path);
        return n.endsWith(".so") || p.endsWith(".so") || p.includes(".so!") || p.includes(".so/");
      }
      isTrustedSystemPath(path) {
        const p = this.normalize(path);
        for (const prefix of this.trustedPrefixes) {
      ...
      isPrivateAppPath(path) {
        const p = this.normalize(path);
        for (const prefix of this.privateAppPrefixes) {
          if (p.startsWith(prefix)) {
            return true;
          }
        }
        if (p.includes("!/lib/") || p.includes("!/lib64/") || p.includes("!/lib/arm64") || p.includes("!/lib/armeabi")) {
          return true;
        }
        return false;
      }
       ...
       * IMPORTANT:
       *
       * این باید قبل از هر classification دیگری باشد.
       * ============================================================
       */
      isAllowedName(name) {
        return this.allowedLibraries.has(this.normalize(name));
      }
      isHighRiskName(name) {
        return this.highRiskNames.has(this.normalize(name));
      }
      safeModule(module2) {
        try {
          if (!module2) {
          ...
          const name = String(module2.name ?? "");
          const path = String(module2.path ?? "");
          if (!this.isSo(name, path)) {
            return null;
          }
          const base = module2.base;
          const size = Number(module2.size);
          if (!base) {
            return null;
          }
          return {
          ...
          this.error(`safeModule failed: ${e}`);
          return null;
        }
      }
      /*
      ...
      enumerateSoModules() {
        ...
        try {
          ...
          this.error(`enumerateModules failed: ${e}`);
          return result;
        }
        for (const raw of modules) {
          const module2 = this.safeModule(raw);
          if (!module2) {
            continue;
          }
          result.push(module2);
      ...
      captureBaseline() {
        this.baseline.clear();
        const modules = this.enumerateSoModules();
        for (const module2 of modules) {
          this.baseline.set(this.moduleKey(module2.name, module2.path), module2);
        }
        this.log(`Baseline modules: ${modules.length}`);
      }
      /*
       * ============================================================
       * CLASSIFICATION
       ...
       * باشد،
       *
       * دیگر suspicious نیست.
       *
       * ============================================================
       */
      classify(module2) {
        const name = this.normalize(module2.name);
        const path = this.normalize(module2.path);
        if (this.isAllowedName(name)) {
          return "WHITELIST_LIBRARY";
        }
        if (this.isTrustedSystemPath(path)) {
          return "SYSTEM_LIBRARY";
        }
        if (this.isHighRiskName(name)) {
          return "HIGH_RISK_NAME";
        }
        if (this.isPrivateAppPath(path)) {
          return "UNKNOWN_PRIVATE_APP_SO";
        }
        return "UNKNOWN_NON_SYSTEM_SO";
      }
      /*
       * ============================================================
       * SUSPICION KEY
       * ============================================================
       */
      suspicionKey(module2) {
        return this.moduleKey(module2.name, module2.path);
      }
      /*
       * ============================================================
       * CLEAR OLD SUSPICION
       * ============================================================
       */
      cleanupSuspicion(activeKeys) {
        for (const key of Array.from(this.suspicious.keys())) {
       ...
       * ============================================================
       */
      markSuspicious(module2, reason) {
        if (this.isAllowedName(module2.name)) {
          this.log(`[WHITELIST] ignored suspicious classification: ${module2.name}`);
          return;
        }
        const key = this.suspicionKey(module2);
        const now = Date.now();
        const existing = this.suspicious.get(key);
        let state;
        if (!existing) {
          state = {
            count: 1,
        ...
        this.suspicious.set(key, state);
        this.warn(`[SUSPICIOUS] ${module2.name} | ${module2.path} | reason=${reason} | confirmation=${state.count}/${this.confirmationsRequired}`);
        if (state.count >= this.confirmationsRequired) {
          this.confirmedViolation(module2, reason);
        }
      }
      /*
       * ============================================================
       ...
       * ============================================================
       */
      confirmedViolation(module2, reason) {
        if (this.isAllowedName(module2.name)) {
          this.log(`[WHITELIST] confirmed violation blocked for ${module2.name}`);
          this.suspicious.delete(this.suspicionKey(module2));
          return;
        }
        const key = this.suspicionKey(module2);
        this.suspicious.delete(key);
        this.log("========================================");
        this.error("SECURITY VIOLATION CONFIRMED");
        this.error(`Module: ${module2.name}`);
        this.error(`Path: ${module2.path}`);
        this.error(`Reason: ${reason}`);
        this.log("========================================");
        if (!this.enforcement) {
          this.warn("Enforcement disabled - no termination");
          return;
        }
       ...
       * ============================================================
       *
       * فقط خود Process بازی.
       * ============================================================
       */
      terminateCurrentProcess() {
        this.warn("[ENFORCEMENT] current game process termination requested");
        try {
          if (typeof frida_java_bridge_1.default !== "undefined" && frida_java_bridge_1.default.available) {
            frida_java_bridge_1.default.perform(() => {
              try {
                const ProcessClass = frida_java_bridge_1.default.use("android.os.Process");
                const pid = ProcessClass.myPid();
                this.error(`[ENFORCEMENT] PID=${pid}`);
                ProcessClass.killProcess(pid);
              } catch (e) {
                this.error(`Java kill failed: ${e}`);
              }
            });
            return;
          }
        } catch (e) {
          this.error(`Java enforcement failed: ${e}`);
        }
      }
      /*
       * ============================================================
      ...
      checkModule(module2) {
        ...
        const classification = this.classify(module2);
        if (classification === "SYSTEM_LIBRARY") {
          return;
        }
        if (classification === "WHITELIST_LIBRARY") {
          return;
        }
        if (classification === "HIGH_RISK_NAME") {
          this.warn(`[HIGH-RISK] ${module2.name} | ${module2.path}`);
          this.markSuspicious(module2, classification);
          return;
        }
        if (classification === "UNKNOWN_PRIVATE_APP_SO") {
          this.markSuspicious(module2, classification);
          return;
        }
        this.markSuspicious(module2, classification);
      ...
      scan() {
        ...
        for (const module2 of modules) {
          activeKeys.add(this.suspicionKey(module2));
          if (this.isAllowedName(module2.name)) {
            continue;
          }
          if (now - this.startTime < this.graceMs) {
            continue;
          }
          this.checkModule(module2);
        }
        this.cleanupSuspicion(activeKeys);
      }
      /*
       * ============================================================
       * MODULE OBSERVER
       * ============================================================
       *
       * اگر بعد از Startup یک Library جدید
       * با dlopen / linker وارد Process شود،
       * observer آن را سریع‌تر از periodic scan
       * می‌بیند.
       * ============================================================
       */
        ...
        try {
          Process.attachModuleObserver({
            onAdded: (module2) => {
              try {
                const record = {
                  name: String(module2.name),
                  path: String(module2.path),
                  base: module2.base,
                  size: Number(module2.size)
                };
                if (!this.isSo(record.name, record.path)) {
                  return;
                }
                if (this.isAllowedName(record.name)) {
                  this.log(`[MODULE+] whitelist: ${record.name}`);
                  return;
                }
                const age = Date.now() - this.startTime;
                if (age < this.graceMs) {
                  this.log(`[MODULE+] ${record.name} | ${record.path} | grace`);
                  return;
                }
                ...
                if (classification === "HIGH_RISK_NAME") {
                  this.warn(`[HIGH-RISK] ${record.name} | ${record.path}`);
                }
                this.markSuspicious(record, classification);
              } catch (e) {
                this.error(`observer onAdded failed: ${e}`);
              }
            },
            onRemoved: (module2) => {
              try {
                const name = String(module2.name);
                const path = String(module2.path);
                this.log(`[MODULE-] ${name} | ${path}`);
              } catch (_) {
              }
            }
          });
          this.observerAttached = true;
          this.log("Module observer enabled");
        } catch (e) {
          this.error(`Module observer failed: ${e}`);
        }
      }
      /*
       * ============================================================
      ...
      startPeriodicScan() {
        ...
        this.scanTimer = setInterval(() => {
          try {
            ...
            this.error(`Periodic scan failed: ${e}`);
          }
        }, this.scanIntervalMs);
      }
      /*
       ...
       * Security.init(any, any)
       *
       * همگی compile می‌شوند.
       * ============================================================
       */
      init(...args) {
        if (this.initialized) {
        ...
        const config2 = args.length > 0 && args[0] && typeof args[0] === "object" ? args[0] : {};
        if (typeof config2.enforcement === "boolean") {
          this.enforcement = config2.enforcement;
        }
        if (Number.isFinite(Number(config2.confirmations)) && Number(config2.confirmations) > 0) {
          this.confirmationsRequired = Math.floor(Number(config2.confirmations));
        }
        if (Number.isFinite(Number(config2.graceMs)) && Number(config2.graceMs) >= 0) {
          this.graceMs = Number(config2.graceMs);
        }
        if (Number.isFinite(Number(config2.scanIntervalMs)) && Number(config2.scanIntervalMs) >= 250) {
          this.scanIntervalMs = Number(config2.scanIntervalMs);
        }
        this.initialized = true;
        this.startTime = Date.now();
        this.log("========================================");
        this.log("LAC SECURITY START");
        this.log(`Whitelist: ${this.allowedLibraries.size}`);
        this.log(`HighRisk: ${this.highRiskNames.size}`);
        this.log(`Grace: ${this.graceMs}ms`);
        this.log(`Confirmations: ${this.confirmationsRequired}`);
        this.log(`Enforcement: ${this.enforcement}`);
        this.log("Whitelist priority: ABSOLUTE");
        this.log("Whitelist path check: BYPASSED");
        this.log("Rename-resistant private SO detection: enabled");
        this.log("========================================");
        this.captureBaseline();
        this.attachModuleObserver();
        this.startPeriodicScan();
        this.log("LAC SECURITY ACTIVE");
      }
      /*
       * ============================================================
       * MANUAL RESCAN
       ...
       */
      rescan() {
        this.log("Manual rescan requested");
        this.scan();
      }
      /*
       * ============================================================
      ...
      getStatus() {
        return {
          ...
          baselineModules: this.baseline.size,
          suspiciousModules: this.suspicious.size,
          observer: this.observerAttached
        };
      }
    };
...
var require_index = __commonJS({
  "index.ts"(exports) {
    ...
    Object.defineProperty(exports, "__esModule", { value: true });
    init_dist();
    var security_1 = __importDefault(require_security());
    var frida_java_bridge_1 = __importDefault((init_frida_java_bridge(), __toCommonJS(frida_java_bridge_exports)));
    var TAG = "[FRIDA-UI]";
    var SERVER_IP = "5.26.26";
    var SERVER_PORT = 2626;
    var MENU_MAIN = 0;
    var MENU_CHARACTER = 3;
    var initialized = false;
    var gtaMenuReady = false;
    var lastMenu = -1;
    var lastNetworkActive = false;
    var characterLocked = false;
    var networkSuppressed = false;
    var waitingAfterExit = false;
    var connectionInProgress = false;
    var connectionStarted = false;
    var characterButton = null;
    var persistentDelegates = [];
    var boundButtonKeys = /* @__PURE__ */ new Set();
    var characterButtonKeys = /* @__PURE__ */ new Set();
    var backMenuButtonKeys = /* @__PURE__ */ new Set();
    var exitButtonKeys = /* @__PURE__ */ new Set();
    var lastButtonPoll = 0;
    var lastStatePoll = 0;
    var networkErrorHookInstalled = false;
    var networkErrorRecoveryInProgress = false;
    var lastCharacterPlayerName = "";
    var characterNamePollTimer = null;
    var characterNamePollAttempts = 0;
    var CHARACTER_NAME_POLL_INTERVAL = 100;
    var CHARACTER_NAME_POLL_MAX_ATTEMPTS = 50;
    var welcomeDispatchedThisSession = false;
    ...
    function javaReady() {
      try {
        return typeof frida_java_bridge_1.default !== "undefined" && frida_java_bridge_1.default.available;
      } catch (_) {
        return false;
      }
    }
    function getActivity() {
      ...
      try {
        frida_java_bridge_1.default.perform(() => {
          try {
            const UnityPlayer = frida_java_bridge_1.default.use("com.unity3d.player.UnityPlayer");
            activity = UnityPlayer.currentActivity.value;
          } catch (e) {
            fail("getActivity failed", e);
          }
        });
      } catch (e) {
        fail("getActivity Java.perform failed", e);
      }
      return activity;
    }
    function uiShow() {
      ...
      try {
        frida_java_bridge_1.default.perform(() => {
          try {
            const Bridge = frida_java_bridge_1.default.use("com.example.gameui.UnityGameUIBridge");
            const activity = getActivity();
            if (!activity) {
              return;
            }
            Bridge.show.overload("android.app.Activity").call(Bridge, activity);
            log("Custom UI SHOW");
          } catch (e) {
            fail("uiShow failed", e);
          }
        });
      } catch (e) {
        fail("uiShow outer failed", e);
      }
    }
    function uiHide() {
      if (!javaReady()) {
      ...
      try {
        frida_java_bridge_1.default.perform(() => {
          try {
            const Bridge = frida_java_bridge_1.default.use("com.example.gameui.UnityGameUIBridge");
            Bridge.hide.overload().call(Bridge);
          } catch (e) {
            fail("uiHide failed", e);
          }
        });
      } catch (e) {
        fail("uiHide outer failed", e);
      }
    }
    function uiSetGameStateOnce(menu, networkActive) {
      if (!javaReady()) {
        return;
      }
      try {
        frida_java_bridge_1.default.perform(() => {
          try {
            const Bridge = frida_java_bridge_1.default.use("com.example.gameui.UnityGameUIBridge");
            Bridge.setGameState.overload("int", "boolean").call(Bridge, menu, networkActive);
          } catch (e) {
            fail(`uiSetGameStateOnce failed ${menu}/${networkActive}`, e);
          }
        });
      } catch (e) {
        fail("uiSetGameStateOnce outer failed", e);
      }
    }
    function uiJoinNotification() {
      if (!javaReady()) {
        return;
      }
      try {
        frida_java_bridge_1.default.perform(() => {
          try {
            const Bridge = frida_java_bridge_1.default.use("com.example.gameui.UnityGameUIBridge");
            Bridge.showJoinNotification.overload().call(Bridge);
          } catch (e) {
            fail("uiJoinNotification failed", e);
          }
        });
      } catch (e) {
        fail("uiJoinNotification outer failed", e);
      }
    }
    function uiShowWelcomeOnce() {
      if (!javaReady()) {
        return false;
      }
      let ok = false;
      try {
        frida_java_bridge_1.default.perform(() => {
          try {
            ...
            Bridge.showWelcomeOnce.overload("android.app.Activity").call(Bridge, activity);
            ok = true;
          } catch (e) {
            fail("uiShowWelcomeOnce failed", e);
          }
        });
      } catch (e) {
        fail("uiShowWelcomeOnce outer failed", e);
      }
      return ok;
    }
    function scheduleWelcomeOnce() {
      if (welcomeDispatchedThisSession) {
        return;
      }
      let attempts = 0;
      const timer = setInterval(() => {
        try {
          attempts++;
          if (uiShowWelcomeOnce()) {
            welcomeDispatchedThisSession = true;
            clearInterval(timer);
            log("WELCOME ONCE DISPATCHED");
            return;
          }
          if (attempts >= 100) {
            clearInterval(timer);
            warn2("WelcomeOnce: Activity not ready after 10s");
          }
        } catch (e) {
          clearInterval(timer);
          fail("scheduleWelcomeOnce failed", e);
        }
      }, 100);
    }
    function uiCharacterEvent() {
      if (!javaReady()) {
        return;
      }
      try {
        frida_java_bridge_1.default.perform(() => {
          try {
            const Bridge = frida_java_bridge_1.default.use("com.example.gameui.UnityGameUIBridge");
            Bridge.onCharacterEvent.overload().call(Bridge);
          } catch (e) {
            fail("uiCharacterEvent failed", e);
          }
        });
      } catch (e) {
        fail("uiCharacterEvent outer failed", e);
      }
    }
    function uiBackMenuEvent() {
      if (!javaReady()) {
        return;
      }
      try {
        frida_java_bridge_1.default.perform(() => {
          try {
            const Bridge = frida_java_bridge_1.default.use("com.example.gameui.UnityGameUIBridge");
            Bridge.onBackMenuEvent.overload().call(Bridge);
          } catch (e) {
            fail("uiBackMenuEvent failed", e);
          }
        });
      } catch (e) {
        fail("uiBackMenuEvent outer failed", e);
      }
    }
    function uiExitEvent() {
      if (!javaReady()) {
        return;
      }
      try {
        frida_java_bridge_1.default.perform(() => {
          try {
            const Bridge = frida_java_bridge_1.default.use("com.example.gameui.UnityGameUIBridge");
            Bridge.onExitEvent.overload().call(Bridge);
          } catch (e) {
            fail("uiExitEvent failed", e);
          }
        });
      } catch (e) {
        fail("uiExitEvent outer failed", e);
      }
    }
    function uiSetPlayerName(name) {
      if (!javaReady()) {
        return;
      }
      try {
        frida_java_bridge_1.default.perform(() => {
          try {
            const Bridge = frida_java_bridge_1.default.use("com.example.gameui.UnityGameUIBridge");
            Bridge.setPlayerName.overload("java.lang.String").call(Bridge, name);
          } catch (e) {
            fail("uiSetPlayerName failed", e);
          }
        });
      } catch (e) {
        fail("uiSetPlayerName outer failed", e);
      }
    }
    function readIl2CppField(object, fieldName) {
      if (!object) {
        return null;
      }
      try {
        return object.field(fieldName).value;
      } catch (_) {
        return null;
      }
    }
    function getCharacterSelect(control) {
      if (!control) {
        return null;
      }
      try {
        return readIl2CppField(control, "_charSelect");
      } catch (_) {
        return null;
      }
    }
    function readCharacterPlayerName(control) {
      try {
        if (!control) {
          log("CHAR NAME: GtaMenuControl = NULL");
          return "";
        }
        const charSelect = getCharacterSelect(control);
        if (!charSelect) {
          log("CHAR NAME: _charSelect = NULL");
          return "";
        }
        const nameInput = readIl2CppField(charSelect, "nameInput");
        if (!nameInput) {
          log("CHAR NAME: nameInput = NULL");
          return "";
        }
        const text = readIl2CppField(nameInput, "m_Text");
        if (!text) {
          log("CHAR NAME: m_Text = NULL");
          return "";
        }
        let content = null;
        try {
          ...
          fail("CHAR NAME: reading text.content failed", e);
          return "";
        }
        if (content === null || content === void 0) {
          log("CHAR NAME: content = NULL");
          return "";
        }
        const value = String(content).trim();
        if (value.length === 0) {
          log("CHAR NAME: content currently EMPTY");
          return "";
        }
        log(`CHAR NAME: VALUE = "${value}"`);
        return value;
      } catch (e) {
        fail("readCharacterPlayerName failed", e);
        return "";
      }
    }
    function updateCharacterPlayerName(control) {
      if (!characterLocked) {
        return;
      }
      if (readNetworkActive()) {
        return;
      }
      const playerName = readCharacterPlayerName(control);
      if (playerName.length === 0) {
        return;
      }
      if (playerName === lastCharacterPlayerName) {
        return;
      }
      lastCharacterPlayerName = playerName;
      log(`CHARACTER PLAYER NAME => ${playerName}`);
      uiSetPlayerName(playerName);
    }
    function startCharacterNamePolling() {
      if (characterNamePollTimer !== null) {
        clearInterval(characterNamePollTimer);
        characterNamePollTimer = null;
      }
      characterNamePollAttempts = 0;
      log("CHAR NAME POLLING START -> Character event only");
      characterNamePollTimer = setInterval(() => {
        try {
          if (!characterLocked) {
            clearInterval(characterNamePollTimer);
            characterNamePollTimer = null;
            log("CHAR NAME POLLING STOP -> Character released");
            return;
          }
          if (readNetworkActive()) {
            clearInterval(characterNamePollTimer);
            characterNamePollTimer = null;
            log("CHAR NAME POLLING STOP -> Network active");
            return;
          }
          characterNamePollAttempts++;
          const control = getGtaMenuControl();
          if (!control) {
            if (characterNamePollAttempts % 5 === 0) {
              log(`CHAR NAME POLL ${characterNamePollAttempts}/${CHARACTER_NAME_POLL_MAX_ATTEMPTS} -> GtaMenuControl NULL`);
            }
          } else {
            updateCharacterPlayerName(control);
            if (lastCharacterPlayerName.length > 0) {
              clearInterval(characterNamePollTimer);
              characterNamePollTimer = null;
              log("CHAR NAME POLLING STOP -> name found");
              return;
            }
          }
          if (characterNamePollAttempts >= CHARACTER_NAME_POLL_MAX_ATTEMPTS) {
            clearInterval(characterNamePollTimer);
            characterNamePollTimer = null;
            warn2("CHAR NAME POLLING TIMEOUT -> name was not found");
            return;
          }
        } catch (e) {
          if (characterNamePollTimer !== null) {
          ...
          fail("Character name polling failed", e);
        }
      }, CHARACTER_NAME_POLL_INTERVAL);
    }
    function stopCharacterNamePolling(reason) {
      if (characterNamePollTimer !== null) {
        clearInterval(characterNamePollTimer);
        characterNamePollTimer = null;
      }
      characterNamePollAttempts = 0;
      log(`CHAR NAME POLLING STOP -> ${reason}`);
    }
    function resetCharacterPlayerName() {
      stopCharacterNamePolling("reset");
      lastCharacterPlayerName = "";
    }
    function releaseNetworkState(reason) {
      if (networkErrorRecoveryInProgress && reason !== "USER_EXIT") {
        return;
      }
      if (reason !== "USER_EXIT") {
        networkErrorRecoveryInProgress = true;
      }
      log(`RELEASE NETWORK STATE -> ${reason}`);
      characterLocked = false;
      resetCharacterPlayerName();
      ...
      uiHide();
      uiExitEvent();
      log("WAITING FOR REAL MAIN + OFFLINE");
    }
    function installJavaHooks() {
      if (!javaReady()) {
        warn2("Java unavailable");
        return;
      }
      try {
        frida_java_bridge_1.default.perform(() => {
          try {
            const Bridge = frida_java_bridge_1.default.use("com.example.gameui.UnityGameUIBridge");
            Bridge.startGame.implementation = function() {
              log("JAVA EVENT -> startGame");
              startGameRequest();
            };
            Bridge.openCharacter.implementation = function() {
              log("JAVA EVENT -> openCharacter");
              requestOriginalCharacter();
            };
            log("Java bridge hooks installed");
          } catch (e) {
            fail("installJavaHooks inner failed", e);
          }
        });
      } catch (e) {
        fail("installJavaHooks outer failed", e);
      }
    }
    function getImage(assemblyName) {
      try {
        return Il2Cpp.domain.assembly(assemblyName).image;
      } catch (_) {
        return null;
      }
    }
    function getButtonClass() {
      try {
        const image = getImage("UnityEngine.UI");
        if (!image) {
          return null;
        }
        return image.class("UnityEngine.UI.Button");
      } catch (_) {
        return null;
      }
    }
    function getGameObject(component) {
      try {
        return component.method("get_gameObject").invoke();
      } catch (_) {
        return null;
      }
    }
    function getName(object) {
      ...
      try {
        const result = object.method("get_name").invoke();
        if (!result) {
          return "";
        }
    ...
    function getButtonName(button) {
      return getName(getGameObject(button));
    }
    function normalizeName(name) {
      return name.trim().toLowerCase().replace(/[\s_\-()[\]]+/g, "");
    }
    function matchesName(actual, names) {
      const value = normalizeName(actual);
      for (const name of names) {
        if (value === normalizeName(name)) {
          return true;
        }
      }
      return false;
    ...
    function getAllButtons() {
      try {
        ...
        const result = Il2Cpp.gc.choose(Button);
        if (!result) {
          return [];
        }
        return Array.from(result);
      } catch (e) {
        fail("getAllButtons failed", e);
        return [];
      }
    }
    function getUnityActionClass() {
      try {
        const image = getImage("UnityEngine.CoreModule");
        if (!image) {
          return null;
        }
        return image.class("UnityEngine.Events.UnityAction");
      } catch (_) {
        return null;
      }
    }
    function createUnityAction(callback) {
      try {
        const UnityAction = getUnityActionClass();
        if (!UnityAction) {
          return null;
        }
        const delegate = Il2Cpp.delegate(UnityAction, () => {
          try {
            callback();
          } catch (e) {
            fail("UnityAction callback exception", e);
          }
        });
        persistentDelegates.push(delegate);
        return delegate;
      } catch (e) {
        fail("createUnityAction failed", e);
        return null;
      }
    }
    function getButtonKey(button) {
      try {
        return String(button.handle);
      } catch (_) {
        return "";
      }
    }
    function bindUnityEvent(button, callback) {
      try {
        const key = getButtonKey(button);
        if (key && boundButtonKeys.has(key)) {
          return true;
        }
        const onClick = button.method("get_onClick").invoke();
        if (!onClick) {
          return false;
        }
        const delegate = createUnityAction(callback);
        if (!delegate) {
          return false;
        }
        onClick.method("AddListener").invoke(delegate);
        if (key) {
          boundButtonKeys.add(key);
        }
        return true;
      } catch (e) {
        fail(`bindUnityEvent failed: ${getButtonName(button)}`, e);
        return false;
      }
    }
    function findCharacterButton(buttons) {
      for (const button of buttons) {
        try {
          if (matchesName(getButtonName(button), [
            "Character",
            "CHARACTER",
            "CharacterButton",
            "CharSelect",
    ...
    function findBackMenuButton(buttons) {
      for (const button of buttons) {
        try {
          if (matchesName(getButtonName(button), [
            "BackMenu",
    ...
    function findExitButton(buttons) {
      for (const button of buttons) {
        try {
          if (matchesName(getButtonName(button), [
            "Exit",
    ...
    function bindCharacterRole(button) {
      const key = getButtonKey(button);
      if (key && characterButtonKeys.has(key)) {
        return;
      }
      if (bindUnityEvent(button, () => {
        log("EVENT -> Character");
        characterButton = button;
        characterLocked = true;
        waitingAfterExit = false;
        networkErrorRecoveryInProgress = false;
        stopCharacterNamePolling("new Character event");
        lastCharacterPlayerName = "";
        uiCharacterEvent();
        startCharacterNamePolling();
      })) {
        if (key) {
          characterButtonKeys.add(key);
        }
        log(`Character event bound: ${getButtonName(button)}`);
      }
    }
    function bindBackMenuRole(button) {
      const key = getButtonKey(button);
      if (key && backMenuButtonKeys.has(key)) {
        return;
      }
      if (bindUnityEvent(button, () => {
        log("EVENT -> BackMenu");
        ...
        if (key) {
          backMenuButtonKeys.add(key);
        }
        log(`BackMenu event bound: ${getButtonName(button)}`);
      }
    }
    function bindExitRole(button) {
      const key = getButtonKey(button);
      if (key && exitButtonKeys.has(key)) {
        return;
      }
      if (bindUnityEvent(button, () => {
        log("EVENT -> Exit");
        releaseNetworkState("USER_EXIT");
      })) {
        if (key) {
          exitButtonKeys.add(key);
        }
        log(`Exit event bound: ${getButtonName(button)}`);
      }
    }
    function scanSpecialEvents(buttons) {
      const character = findCharacterButton(buttons);
      if (character) {
        characterButton = character;
        bindCharacterRole(character);
      }
      const backMenu = findBackMenuButton(buttons);
      if (backMenu) {
        bindBackMenuRole(backMenu);
      }
      const exit = findExitButton(buttons);
      if (exit) {
        bindExitRole(exit);
      }
    }
    function requestOriginalCharacter() {
      if (!gtaMenuReady) {
        warn2("Character blocked: GtaMenuControl not ready");
        return;
      }
      try {
        const buttons = getAllButtons();
        const live = findCharacterButton(buttons);
        if (live) {
          characterButton = live;
        }
        if (!characterButton) {
          warn2("Character button not found");
          return;
        }
        characterButton.method("Press").invoke();
        log("Original Character Press() invoked");
      } catch (e) {
        fail("Original Character Press() failed", e);
      }
    }
    function getGtaMenuControl() {
      try {
        const image = getImage("Assembly-CSharp");
        if (!image) {
          return null;
        }
        const klass = image.class("GtaMenuControl");
        if (!klass) {
          return null;
        }
        const singleton = klass.method("get_Instance").invoke();
        if (!singleton) {
          return null;
        }
        try {
          return new Il2Cpp.Object(singleton.handle);
        } catch (_) {
          return singleton;
        }
      } catch (_) {
    ...
    function readCurrentMenu(control) {
      for (const name of [
        "get_currentMenu",
        "get_CurrentMenu",
        "get_menu",
        ...
        try {
          const value = control.method(name).invoke();
    ...
    function getNetworkManager() {
      try {
        const image = getImage("Mirror");
        if (!image) {
          return null;
        }
        const NetworkManager = image.class("Mirror.NetworkManager");
        if (!NetworkManager) {
          return null;
        }
        const singleton = NetworkManager.method("get_singleton").invoke();
        if (!singleton) {
          return null;
        }
        return new Il2Cpp.Object(singleton.handle);
    ...
    function readNetworkActive() {
      try {
        const image = getImage("Mirror");
        if (!image) {
          return false;
        }
        const NetworkClient = image.class("Mirror.NetworkClient");
        if (!NetworkClient) {
          return false;
        }
        try {
          if (!!NetworkClient.method("get_isConnected").invoke()) {
            return true;
          }
        } catch (_) {
        }
        for (const methodName of [
          ...
          try {
            if (!!NetworkClient.method(methodName).invoke()) {
              return true;
            }
          } catch (_) {
          }
    ...
    function getSystemUri() {
      try {
        for (const assembly of Il2Cpp.domain.assemblies) {
          try {
            const klass = assembly.image.tryClass("System.Uri");
            if (klass) {
              return klass;
            }
          } catch (_) {
    ...
    function directConnect() {
      const gta = getGtaMenuControl();
      if (!gta) {
        warn2("directConnect: GtaMenuControl not ready");
        return false;
      }
      const manager = getNetworkManager();
      if (!manager) {
        warn2("directConnect: NetworkManager not ready");
        return false;
      }
      const Uri = getSystemUri();
      if (!Uri) {
        warn2("directConnect: System.Uri not ready");
        return false;
      }
      try {
        const uri = Uri.alloc();
        const address = `kcp://${SERVER_IP}:${SERVER_PORT}`;
        uri.method(".ctor").overload("System.String").invoke(Il2Cpp.string(address));
        manager.method("StartClient").overload("System.Uri").invoke(uri);
        log(`DIRECT CONNECT INVOKED -> ${address}`);
        return true;
      } catch (e) {
        fail("directConnect failed", e);
        return false;
      }
    }
    function startGameRequest() {
      if (connectionInProgress || networkSuppressed) {
        warn2("Start Game ignored: network/connection already active");
        return;
      }
      ...
      networkErrorRecoveryInProgress = false;
      uiHide();
      waitForGtaMenuAndConnect();
    }
    function waitForGtaMenuAndConnect() {
      if (getGtaMenuControl()) {
        beginConnection();
        return;
      }
      let attempts = 0;
      const timer = setInterval(() => {
        try {
          ...
          if (attempts >= 100) {
            clearInterval(timer);
            connectionInProgress = false;
            warn2("GtaMenuControl timeout; UI remains hidden");
          }
        } catch (e) {
          clearInterval(timer);
          connectionInProgress = false;
          fail("waitForGtaMenuAndConnect failed", e);
        }
      }, 100);
    }
    function beginConnection() {
      uiJoinNotification();
      const success = directConnect();
      if (success) {
        connectionStarted = true;
        connectionInProgress = false;
        log("StartClient invocation completed; network suppression locked");
        return;
      }
      retryManagerAvailability();
    }
    function retryManagerAvailability() {
      let attempts = 0;
      const timer = setInterval(() => {
        try {
          attempts++;
          if (readNetworkActive()) {
            ...
            networkSuppressed = true;
            uiHide();
            log("Network became active");
            return;
          }
          if (!connectionStarted && getNetworkManager()) {
            if (directConnect()) {
              connectionStarted = true;
              connectionInProgress = false;
              clearInterval(timer);
              networkSuppressed = true;
              uiHide();
              log("Delayed StartClient invocation completed");
              return;
            }
          }
          if (attempts >= 100) {
            clearInterval(timer);
            connectionInProgress = false;
            warn2("Connection availability timeout; waiting for real NetworkManager error/state");
          }
        } catch (e) {
          clearInterval(timer);
          connectionInProgress = false;
          fail("retryManagerAvailability failed", e);
        }
      }, 100);
    }
    function installCustomNetworkErrorHook() {
      if (networkErrorHookInstalled) {
        return;
      }
      try {
        const image = getImage("Assembly-CSharp");
        if (!image) {
          warn2("OnClientError hook: Assembly-CSharp not ready");
          return;
        }
        const klass = image.class("CustomNetworkManager");
        if (!klass) {
          warn2("OnClientError hook: CustomNetworkManager not found");
          return;
        }
        const method = klass.method("OnClientError").overload("Mirror.TransportError", "System.String");
        if (!method) {
          warn2("OnClientError hook: overload not found");
          return;
        }
        const address = method.virtualAddress;
        if (!address) {
          warn2("OnClientError hook: native address unavailable");
          return;
        }
        Interceptor.attach(address, {
          onEnter(args) {
            try {
              const networkFlow = readNetworkActive() || connectionInProgress || connectionStarted || networkSuppressed;
              if (!networkFlow) {
                return;
              }
              let transportError = "Unknown";
              try {
                transportError = String(args[1].toInt32());
              } catch (_) {
              }
              log("========================================");
              log("CustomNetworkManager::OnClientError");
              log(`transportError=${transportError}`);
              log(`networkActive=${readNetworkActive()}`);
              log(`connectionInProgress=${connectionInProgress}`);
              log(`connectionStarted=${connectionStarted}`);
              log(`waitingAfterExit=${waitingAfterExit}`);
              if (!networkErrorRecoveryInProgress) {
                releaseNetworkState(`NETWORK_ERROR:${transportError}`);
              } else {
                log("Network error recovery already active");
              }
              log("========================================");
            } catch (e) {
              fail("OnClientError onEnter failed", e);
            }
          }
        });
        networkErrorHookInstalled = true;
        log("CustomNetworkManager::OnClientError hook installed");
      } catch (e) {
        fail("installCustomNetworkErrorHook failed", e);
      }
    }
    function updateGameState(menu, networkActive) {
      const menuChanged = menu !== lastMenu;
      const networkChanged = networkActive !== lastNetworkActive;
      lastMenu = menu;
      lastNetworkActive = networkActive;
      if (networkActive) {
        if (!networkSuppressed || networkChanged) {
          networkSuppressed = true;
          characterLocked = false;
          resetCharacterPlayerName();
          uiHide();
          log("NETWORK ACTIVE -> Custom UI hidden");
        }
        return;
      }
      if (waitingAfterExit) {
        if (menu === MENU_MAIN && !networkActive) {
          waitingAfterExit = false;
          networkSuppressed = false;
          connectionStarted = false;
          connectionInProgress = false;
          characterLocked = false;
          resetCharacterPlayerName();
          networkErrorRecoveryInProgress = false;
          uiShow();
          uiSetGameStateOnce(MENU_MAIN, false);
          log("EXIT/NETWORK ERROR -> REAL MAIN -> Custom UI restored");
        } else {
          uiHide();
        }
        return;
      ...
      if (characterLocked) {
        if (menu === MENU_CHARACTER || menuChanged) {
          uiShow();
        }
        return;
      }
      if (menu === MENU_MAIN) {
        if (menuChanged || networkChanged) {
          uiShow();
          uiSetGameStateOnce(MENU_MAIN, false);
        }
        return;
      }
      if (menu === MENU_CHARACTER) {
        characterLocked = true;
        uiShow();
        uiSetGameStateOnce(MENU_CHARACTER, false);
        if (characterNamePollTimer === null) {
          startCharacterNamePolling();
        }
        return;
    ...
    function pollGameState() {
      ...
      gtaMenuReady = true;
      const menu = readCurrentMenu(control);
      if (menu === null) {
        return;
      }
      const network = readNetworkActive();
      updateGameState(menu, network);
    }
    function pollButtons() {
      const now = Date.now();
      if (now - lastButtonPoll < 700) {
        return;
      }
      lastButtonPoll = now;
      if (!isGameReady()) {
      ...
      disableOnlyRequestedButtons(buttons);
      scanSpecialEvents(buttons);
    }
    function disableOnlyRequestedButtons(buttons) {
      const targets = [
        "LACEDITOR",
        "COMMUNITY",
        "DOCUMENT"
      ...
      for (const button of buttons) {
        try {
          ...
          try {
            const interactable = button.method("get_interactable").invoke();
            if (interactable) {
              button.method("set_interactable").invoke(false);
              log(`disabled ONLY: ${name}`);
            }
          } catch (_) {
          }
        } catch (_) {
    ...
    function initializeUi() {
      ...
      const menu = readCurrentMenu(control);
      if (menu === null) {
        warn2("Initial UI: currentMenu unresolved");
        return;
      }
      const network = readNetworkActive();
      if (network) {
      ...
      if (menu === MENU_CHARACTER) {
        ...
        uiShow();
        uiSetGameStateOnce(MENU_CHARACTER, false);
        log("Initial Character UI shown");
        startCharacterNamePolling();
        return;
      }
      uiHide();
    ...
    function waitForInitialUi() {
      ...
      const timer = setInterval(() => {
        try {
          ...
          if (attempts >= 100) {
            clearInterval(timer);
            warn2("Initial GtaMenuControl wait timeout");
          }
        } catch (e) {
          clearInterval(timer);
          fail("waitForInitialUi failed", e);
        }
      }, 100);
    }
    function initialize2() {
      ...
      initialized = true;
      log("========================================");
      log("FRIDA GAME UI START");
      log(`Unity: ${Il2Cpp.unityVersion}`);
      log(`Server: ${SERVER_IP}:${SERVER_PORT}`);
      log("Master gate: GtaMenuControl");
      log("Disabled ONLY: LACEDITOR, COMMUNITY, DOCUMENT");
      log("Character = Event + locked UI");
      log("Character Name = IL2CPP _charSelect.nameInput.m_Text");
      log("Character Name polling = ONLY while Character is open");
      log("Character Name polling delay = 100ms");
      log("Character Name polling max = 5 seconds");
      log("BackMenu = Event releases Character");
      log("Exit = Event releases Network suppression");
      log("Network/Connecting = Custom UI hidden");
      log("Network Error = same recovery path as Exit");
      log("========================================");
      security_1.default.init({
        startupGraceMs: 15e3,
        ...
        verbose: true,
        onDetection: (info) => {
          console.error(`[LAC-SEC] CONFIRMED ${info.type}: ${info.moduleName ?? ""} ${info.reason}`);
        }
      });
      installJavaHooks();
      scheduleWelcomeOnce();
      installCustomNetworkErrorHook();
      initializeUi();
      setInterval(() => {
        try {
          pollGameState();
        } catch (e) {
          fail("pollGameState exception", e);
        }
      }, 50);
      setInterval(() => {
        try {
    ...
    rpc.exports = {
      connect() {
        if (networkSuppressed || connectionInProgress) {
          warn2("RPC connect ignored: already suppressed/connecting");
          return false;
        }
        networkSuppressed = true;
        uiHide();
      ...
      showui() {
        if (networkSuppressed || waitingAfterExit) {
          warn2("showui blocked by network/exit state");
          return;
        }
        initializeUi();
      },
      ...
      /*
       * ========================================================
       * WELCOME ONCE (MANUAL TEST)
       * ========================================================
       *
       * برای تست دستی از Frida REPL:
       *
       *     rpc.exports.showwelcome()
       *
       * این باعث می‌شود Frida دوباره تلاش کند
       * حتی اگر قبلاً در همین session dispatch شده باشد.
       *
       * ولی همچنان اگر کاربر قبلاً "باشه" را زده باشد،
       * Kotlin چیزی نشان نمی‌دهد.
       * ========================================================
       */
      showwelcome() {
        welcomeDispatchedThisSession = false;
        scheduleWelcomeOnce();
      },
      state() {
        try {
          const control = getGtaMenuControl();
          if (!control) {
            log("STATE GtaMenuControl=null");
            return;
          }
          const menu = readCurrentMenu(control);
          const network = readNetworkActive();
          log(`STATE menu=${menu} network=${network} characterLocked=${characterLocked} networkSuppressed=${networkSuppressed} waitingAfterExit=${waitingAfterExit} connectionStarted=${connectionStarted} connectionInProgress=${connectionInProgress} networkErrorRecoveryInProgress=${networkErrorRecoveryInProgress} characterNamePolling=${characterNamePollTimer !== null} lastCharacterPlayerName=${lastCharacterPlayerName} welcomeDispatched=${welcomeDispatchedThisSession}`);
        } catch (e) {
          fail("RPC state failed", e);
        }
      }
    };
  }
